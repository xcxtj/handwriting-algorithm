...
生成 ETag 值时，并没有统一的算法规则，而仅仅是由服务器来分配。分布式服务器系统，一模一样的一个文件的Etag值可能不一样。计算**性能损耗**
...
名字不能改  每个域名下Cookie的数量不能超过20个，大小<4kb，它是一种纯文本文件 无法跨域名,每次发起HTTP请求都会携带 请求头。子域可以**共享使用的**（**靠的是 domain）**
...
302 临时重定向，会错误的把post改为get  303只允许重定向到get 307不允许重定向到get
...
签名：用一种hash对证书（公钥，hash算法，签发者）加密得到摘要，认证中心ca用私钥加密得到签名，发送数字证书（签名加原始信息）。浏览器用同样的hash得到摘要，用内置的公钥解密后对比。最后得到服务器的非对称公钥
...
![image-20240823101419722](C:\Users\XTJ\AppData\Roaming\Typora\typora-user-images\image-20240823101419722.png)
